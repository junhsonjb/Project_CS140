...
//		public CPU(int accumulator, int counter) {
...
//		}
	...
	//Now, the fields for MachineModel, and thus the rest of the actual class
	...
	public MachineModel(boolean withGUI, HaltCallback cb) {
		...
		ACTION.put(opcodes.get("NOT"), instr -> {
			...
			cpu.pc++; //incrementing the program counter
		});
		...
		ACTION.put(opcodes.get("HALT"), instr -> {
			...
			halt(); //not sure if I'm calling this the right way
		...
		ACTION.put(opcodes.get("STO"), instr -> {
			...
			} else if (flags == 4) { //indirect
				memory.setData(memory.getData(instr.arg), cpu.accum);
			} else { //including immeadiate addressing among other things
				String fString = "(" + (flags%8 > 3?"1":"0") + (flags%4 > 1?"1":"0") + ")";
				throw new IllegalInstructionException("Illegal flags for this instruction: " + fString);
			}
			cpu.pc++;
	...
	// New halt method utilizing the new HaltCallback
	public void halt() {
	...
	//Delegate methods:
	// all comments, return values, privacy types, and parameters come directly from the code block provided by the professor
	
	// calls the corresponding method in memory
	...
	// calls the corresponding value from memory << CORRECTION <<
	...
	// the method that drives the execution of instructions
	// In a try/catch:
	// get the Instruction instr at cpu.pc using getCode
	// call Instruction.checkParity(instr);
	// "execute" the lambda expression for instr: ACTION.get(instr.opcode/8).accept(instr);
	// The catch is for any Exception e. The exception handler code calls halt() and then calles
	// throw e; to propagate the exception to the GUI, which will call step.
	public void step() {
		try {
